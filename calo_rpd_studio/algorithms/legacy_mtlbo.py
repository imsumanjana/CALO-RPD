...
class LegacyMTLBOOptimizer(BaseOptimizer):
    ...
    def run(self):
        ...
        while self.iteration<self.config.max_iterations and self.can_evaluate():
            self.iteration+=1;best=p[self.order(ev)[0]];mean=p.mean(0);cand=np.clip(p+self.rng.normal(0,1,p.shape)*(best-mean),0,1);ce=self.evaluate_population(cand)
            for i,e in enumerate(ce):
                if better(e,ev[i]):p[i]=cand[i];ev[i]=e
            self.record()
