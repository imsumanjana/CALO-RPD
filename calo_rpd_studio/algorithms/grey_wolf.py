...
class GreyWolfOptimizer(BaseOptimizer):
    ...
    def run(self):
        ...
        while self.iteration < self.config.max_iterations and self.can_evaluate():
            self.iteration += 1
            order = self.order(ev)
            leaders = [p[order[min(k, len(order) - 1)]] for k in range(3)]
            a = 2 * (1 - min(self.iteration / max(self.config.max_iterations, 1), 1))
            xs = []
            for leader in leaders:
                A = 2 * a * self.rng.random(p.shape) - a
                C = 2 * self.rng.random(p.shape)
                xs.append(leader - A * np.abs(C * leader - p))
            p = np.clip(sum(xs) / 3, 0, 1)
            ev = self.evaluate_population(p)
            self.record()
