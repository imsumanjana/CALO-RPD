...
class GrasshopperOptimizer(BaseOptimizer):
    ...
    @staticmethod
    def _social_interaction(distance, f=0.5, length_scale=1.5):
        return f * np.exp(-distance / length_scale) - np.exp(-distance)
    ...
    def run(self):
        ...
        while self.iteration < self.config.max_iterations and self.can_evaluate():
            self.iteration += 1
            target = p[self.order(ev)[0]]
            c = 1 - 0.99999 * min(self.iteration / max(self.config.max_iterations, 1), 1)
            new = np.zeros_like(p)
            for i in range(len(p)):
                ...
                for j in range(len(p)):
                    ...
                    social += self._social_interaction(10 * dist) * (diff / dist)
