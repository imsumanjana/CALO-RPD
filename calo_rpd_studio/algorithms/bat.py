...
class BatOptimizer(BaseOptimizer):
    ...
    def run(self):
        ...
        loud = float(self.config.parameters.get("loudness", 0.9))
        ...
        while self.iteration < self.config.max_iterations and self.can_evaluate():
            ...
            for i in range(len(p)):
                if not self.can_evaluate():
                    break
                freq = self.rng.uniform(0, 2)
                v[i] += (p[i] - best) * freq
                x = np.clip(p[i] + v[i], 0, 1)
                if self.rng.random() > pulse:
                    x = np.clip(best + self.rng.normal(0, 0.01, self.problem.dimension), 0, 1)
                e = self.evaluate(x)
                if e is not None and better(e, ev[i]) and self.rng.random() < loud:
                    p[i] = x
                    ev[i] = e
            self.record()
