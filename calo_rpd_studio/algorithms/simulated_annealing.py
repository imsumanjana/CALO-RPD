...
class SimulatedAnnealingOptimizer(BaseOptimizer):
    ...
    def run(self):
        ...
        x = self.rng.random(self.problem.dimension)
        ev = self.evaluate(x)
        temp = float(self.config.parameters.get("temperature", 1.0))
        cool = float(self.config.parameters.get("cooling", 0.995))
        ...
        while self.iteration < self.config.max_iterations and self.can_evaluate():
            self.iteration += 1
            c = np.clip(x + self.rng.normal(0, scale, self.problem.dimension), 0, 1)
            ce = self.evaluate(c)
            if ce is None:
                break
            delta = (
                (ce.value - ev.value)
                if ce.feasible == ev.feasible
                else (ce.violation - ev.violation)
            )
            if better(ce, ev) or self.rng.random() < np.exp(-max(delta, 0) / max(temp, 1e-12)):
                x = c
                ev = ce
            temp *= cool
            self.record()
