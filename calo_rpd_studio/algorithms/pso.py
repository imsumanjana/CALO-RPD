...
class PSOOptimizer(BaseOptimizer):
    ...
    def run(self):
        ...
        v = self.rng.uniform(-0.1, 0.1, p.shape)
        e = self.evaluate_population(p)
        if len(e) < len(p):
            return self.finalize(p[: len(e)], started=started)
        pb = p.copy()
        pe = list(e)
        w = float(self.config.parameters.get("inertia", 0.7298))
