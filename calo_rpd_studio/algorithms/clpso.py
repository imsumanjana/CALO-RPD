            ...
            for i in range(n):
                if stale[i] >= refresh:
                    for j in range(d):
                        ...
                        winner = a if better(pe[a], pe[b]) else b
                        exemplar[i, j] = pb[winner, j]
                    stale[i] = 0
            w = 0.9 - 0.5 * min(self.iteration / max(self.config.max_iterations, 1), 1)
            v = w * v + c * self.rng.random(p.shape) * (exemplar - p)
            p = np.clip(p + v, 0, 1)
            ev = self.evaluate_population(p)
