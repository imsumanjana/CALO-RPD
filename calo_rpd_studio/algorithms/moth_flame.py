...
class MothFlameOptimizer(BaseOptimizer):
    ...
    def run(self):
        ...
        while self.iteration < self.config.max_iterations and self.can_evaluate():
            ...
            flames = combo[order]
            fe = [all_ev[i] for i in order]
            nf = max(
                1,
                int(
                    round(
                        len(p) - (len(p) - 1) * self.iteration / max(self.config.max_iterations, 1)
                    )
                ),
            )
            new = []
            for i, x in enumerate(p):
                flame = flames[min(i, nf - 1)]
                dist = np.abs(flame - x)
                t = self.rng.uniform(-1, 1, self.problem.dimension)
                new.append(np.clip(dist * np.exp(t) * np.cos(2 * np.pi * t) + flame, 0, 1))
            p = np.asarray(new)
            ev = self.evaluate_population(p)
            self.record()
