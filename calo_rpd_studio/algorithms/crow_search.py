...
class CrowSearchOptimizer(BaseOptimizer):
    ...
    def run(self):
        ...
        mem = p.copy()
        me = list(ev)
        ap = float(self.config.parameters.get("awareness_probability", 0.1))
        fl = float(self.config.parameters.get("flight_length", 2.0))
        while self.iteration < self.config.max_iterations and self.can_evaluate():
            self.iteration += 1
            new = []
            for i in range(len(p)):
                j = int(self.rng.integers(len(p)))
                x = (
                    p[i] + self.rng.random() * fl * (mem[j] - p[i])
                    if self.rng.random() > ap
                    else self.rng.random(self.problem.dimension)
                )
                new.append(np.clip(x, 0, 1))
            p = np.asarray(new)
            ev = self.evaluate_population(p)
