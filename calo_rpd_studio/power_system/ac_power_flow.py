...
class PowerFlowResult:
    ...
    def total_loss_mw(self):
        return float("inf") if self.branch is None else self.branch.total_loss_mw


def _types(case):
    t = case.bus[:, BUS_TYPE].astype(int)
    ref = np.where(t == REF)[0]
    pv = np.where(t == PV)[0]
    pq = np.where(t == PQ)[0]
    if ref.size != 1:
...
def _sbus(case):
    ...
    pg = np.zeros(case.n_bus)
    qg = np.zeros(case.n_bus)
    for g in np.where(case.gen[:, GEN_STATUS] > 0)[0]:
        b = idx[int(case.gen[g, GEN_BUS])]
        pg[b] += case.gen[g, PG]
        qg[b] += case.gen[g, QG]
...
def _v0(case):
    vm = case.bus[:, VM].copy()
    va = np.deg2rad(case.bus[:, VA].copy())
    idx = case.bus_index_map()
    for gen in case.gen[case.gen[:, GEN_STATUS] > 0]:
        vm[idx[int(gen[GEN_BUS])]] = gen[VG]
    return vm * np.exp(1j * va)


def _required(case, v, y):
    inj = v * np.conj(y @ v) * case.base_mva
    return inj.real + case.bus[:, PD], inj.imag + case.bus[:, QD]
...
def _update_outputs(case, pg, qg):
    for i, bus_number in enumerate(case.bus[:, BUS_I].astype(int)):
        ...
        if is_ref:
            fixed = float(np.sum(case.gen[gens[1:], PG])) if gens.size > 1 else 0.0
...
def run_ac_power_flow(input_case, options: PowerFlowOptions | None = None):
    ...
    for qround in range(options.max_q_limit_rounds + 1):
        ...
        if not options.enforce_q_limits:
            ...
            br = calculate_branch_flows(case, v, adm)
            return PowerFlowResult(
                True,
                case,
                v,
        ...
        violations = []
        for bi in pv:
            busnum = int(case.bus[bi, BUS_I])
            qmin, qmax = aggregate_q_limits(case, busnum)
            req = float(qg[bi])
            if req > qmax + options.q_limit_tolerance_mvar:
                violations.append((bi, qmax))
            elif req < qmin - options.q_limit_tolerance_mvar:
                violations.append((bi, qmin))
        if not violations:
            _update_outputs(case, pg, qg)
            br = calculate_branch_flows(case, v, adm)
            return PowerFlowResult(
        ...
        for bi, limit in violations:
            busnum = int(case.bus[bi, BUS_I])
            distribute_reactive_power(case, busnum, limit)
            case.bus[bi, BUS_TYPE] = PQ
            warnings.append(
                f"Bus {busnum} converted from PV to PQ at aggregate Q limit {limit:g} MVAr."
            )
        adm = build_ybus(case)
