...
class PowerSystemCase:
    ...
    def __post_init__(self):
        ...
        self.gen = np.asarray(self.gen, dtype=float)
    ...
    def bus_index_map(self):
        return {int(v): i for i, v in enumerate(self.bus[:, BUS_I])}

    def checksum(self):
        h = hashlib.sha256()
        h.update(np.asarray([self.base_mva], dtype=np.float64).tobytes())
        for arr in (self.bus, self.gen, self.branch):
