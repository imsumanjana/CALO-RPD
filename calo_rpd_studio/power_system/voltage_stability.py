...
def kessel_glavitsch_l_index(case, voltage):
    ...
    if not load.size or not gen.size:
        return LIndexResult(np.zeros(load.size), load, 0.0)
    y = build_ybus(case).ybus.toarray()
    yll = y[np.ix_(load, load)]
    ylg = y[np.ix_(load, gen)]
    try:
        f = -np.linalg.solve(yll, ylg)
    except np.linalg.LinAlgError:
        return LIndexResult(np.full(load.size, np.inf), load, float("inf"))
    values = np.abs(1 - (f @ voltage[gen]) / voltage[load])
    return LIndexResult(values, load, float(np.max(values)))
