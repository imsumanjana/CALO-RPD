...
def descriptive_statistics(values, confidence=0.95):
    ...
    return {
        "count": int(len(x)), "best": float(np.min(x)), "mean": mean,
        "median": float(np.median(x)), "worst": float(np.max(x)), "std": std,
        "variance": float(std ** 2), "iqr": float(np.percentile(x, 75) - np.percentile(x, 25)),
