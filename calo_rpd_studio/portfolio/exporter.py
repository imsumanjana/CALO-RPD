...
class PortfolioExporter:
    ...
    def _bundle_candidates(out: Path, archive: Path, manifest_path: Path) -> list[Path]:
        ...
        return sorted(
            (
                ...
                if path.resolve(strict=False) not in excluded and not path.name.endswith(".tmp")
            ),
            ...
            with zipfile.ZipFile(
                ...
                for index, candidate in enumerate(candidates, start=1):
                    if cancel_callback and cancel_callback():
                        raise PortfolioExportCancelled(
                            "Safe pause requested during reproducibility bundle creation"
                        )
                    compression = (
                        zipfile.ZIP_STORED
    ...
    def _constraint_decomposition(self, row: dict, path: Path) -> str:
        ...
        ax.set_title("Constraint-violation decomposition")
        ax.grid(True, alpha=0.35)
        ax.legend()
        return self._save_figure(fig, path)
    ...
    def _solution_scenario(self, row: dict) -> dict:
        data = self._load_result(row)
        state = (data.get("metadata", {}) or {}).get("solution_state", {}) or {}
    ...
    def _voltage_profile(self, row: dict, path: Path) -> str:
        ...
        buses = np.asarray(
            scenario.get("bus_numbers", np.arange(len(scenario.get("vm_pu", []))) + 1)
        )
        vm = np.asarray(scenario.get("vm_pu", []), dtype=float)
        if vm.size == 0:
            raise ValueError("Voltage magnitudes are unavailable")
        fig, ax = plt.subplots(figsize=(8.0, 4.8))
        ax.plot(buses, vm, marker="o")
        ax.axhline(1.0, linestyle="--", linewidth=1)
        ax.set_xlabel("Bus number")
        ax.set_ylabel("Voltage magnitude (p.u.)")
        ax.set_title(f"Optimized bus-voltage profile — {row['algorithm']}")
        ax.grid(True, alpha=0.35)
        return self._save_figure(fig, path)
    ...
    def _heatmap(self, values: np.ndarray, xlabels, title: str, xlabel: str, path: Path) -> str:
        ...
        if values.size == 0:
            raise ValueError("Heatmap data are unavailable")
        ...
        ax.set_xticks(ticks)
        ax.set_xticklabels([str(xlabels[i]) for i in ticks], rotation=45, ha="right")
        fig.colorbar(image, ax=ax, shrink=0.75)
        return self._save_figure(fig, path)

    def _branch_loading(self, row: dict, path: Path, heatmap: bool = False) -> str:
        scenario = self._solution_scenario(row)
        loading = np.asarray(scenario.get("loading_percent", []), dtype=float)
        if loading.size == 0:
            raise ValueError("Branch-loading state is unavailable")
        labels = [
            f"{a}-{b}"
            for a, b in zip(
                scenario.get("branch_from_bus", range(len(loading))),
                scenario.get("branch_to_bus", range(len(loading))),
            )
        ]
        if heatmap:
            return self._heatmap(
                loading, labels, f"Branch-loading heatmap — {row['algorithm']}", "Branch", path
            )
        fig, ax = plt.subplots(figsize=(9.0, 5.0))
        ax.bar(np.arange(len(loading)), loading)
        ax.axhline(100, linestyle="--", linewidth=1)
        ax.set_xlabel("Branch index")
        ax.set_ylabel("Loading (%)")
        ax.set_title(f"Optimized branch loading — {row['algorithm']}")
        ax.grid(True, axis="y", alpha=0.35)
        return self._save_figure(fig, path)

    def _controls(self, row: dict, path: Path) -> str:
        data = self._load_result(row)
        controls = data.get("decoded_controls", {}) or {}
        labels, values = [], []
        ...
        if not values:
            raise ValueError("Decoded scalar controls are unavailable")
        fig, ax = plt.subplots(figsize=(max(8.0, len(values) * 0.35), 5.0))
        ax.bar(np.arange(len(values)), values)
        ax.set_xticks(np.arange(len(values)))
        ax.set_xticklabels(labels, rotation=65, ha="right")
        ax.set_title(f"Optimized ORPD controls — {row['algorithm']}")
        ax.set_ylabel("Physical control value")
        return self._save_figure(fig, path)
    ...
    def _median_convergence(self, rows: list[dict], path: Path, uncertainty: bool = False) -> str:
        ...
        if grid.size == 0 or not aligned:
            raise ValueError("Repeated feasible convergence histories are unavailable")
        ...
        ax.set_title("Median feasible convergence" + (" with IQR" if uncertainty else ""))
        ax.grid(True, alpha=0.35)
        ax.legend()
        return self._save_figure(fig, path)
    ...
    def _distribution(self, frame: pd.DataFrame, path: Path, violin: bool = False) -> str:
        grouped = [
            group["objective"].dropna().to_numpy() for _, group in frame.groupby("algorithm")
        ]
        labels = [name for name, _ in frame.groupby("algorithm")]
        if not grouped:
            raise ValueError("Repeated objective records are unavailable")
        fig, ax = plt.subplots(figsize=(max(7.5, len(labels) * 0.9), 5.2))
        if violin:
            ax.violinplot(grouped, showmedians=True)
        ...
        ax.set_title("Final feasible objective distribution")
        ax.grid(True, axis="y", alpha=0.35)
        return self._save_figure(fig, path)
    ...
    def _feasible_probability(self, frame: pd.DataFrame, path: Path) -> str:
        ...
        fig, ax = plt.subplots(figsize=(max(7.2, len(rates) * 0.8), 4.8))
        ax.bar(rates.index, rates.values)
        ax.set_ylim(0, 105)
        ax.set_ylabel("Feasible runs (%)")
        ax.set_title("Feasible-run probability")
    ...
    def _evaluations_to_feasibility(self, rows: list[dict], path: Path) -> str:
        ...
        for row in rows:
            data = self._load_result(row)
            first = (data.get("metadata", {}) or {}).get("first_feasible_evaluation")
            if first is not None:
                records.append((row["algorithm"], float(first)))
        if not records:
            raise ValueError(
                "No run reached feasibility or the first-feasible history is unavailable"
            )
        frame = pd.DataFrame(records, columns=["algorithm", "evaluations"])
        groups = [g["evaluations"].to_numpy() for _, g in frame.groupby("algorithm")]
        labels = [n for n, _ in frame.groupby("algorithm")]
        fig, ax = plt.subplots(figsize=(max(7.2, len(labels) * 0.8), 4.8))
    ...
    def _objective_violation_scatter(self, rows: list[dict], path: Path) -> str:
        ...
        for row in rows:
            ...
            points.append(
                (
                    row["algorithm"],
                    float(data.get("total_constraint_violation", np.nan)),
                    float(data.get("best_objective", np.nan)),
                )
            )
        frame = pd.DataFrame(points, columns=["algorithm", "violation", "objective"]).dropna()
        if frame.empty:
            raise ValueError("Objective/violation records are unavailable")
        ...
        ax.set_xlabel("Final normalized constraint violation")
        ax.set_ylabel("Final objective")
        ax.set_title("Objective–violation relationship")
        ax.grid(True, alpha=0.35)
        ax.legend()
        return self._save_figure(fig, path)
    ...
    def _calo_regime_plot(self, rows: list[dict], path: Path) -> str:
        ...
        ax.set_ylabel("Cognitive regime")
        ax.set_title("CALO cognitive-regime timeline")
        ax.grid(True, alpha=0.35)
        return self._save_figure(fig, path)
    ...
    def _paired_merits(self, rows: list[dict]) -> tuple[list[str], list[int], np.ndarray]:
        ...
        lookup = {
            (str(row["algorithm"]), int(row["run_index"])): self._load_result(row) for row in rows
        }
        complete = [
            index for index in run_indices if all((name, index) in lookup for name in algorithms)
        ]
        ...
        finite_any = [
            float(data.get("best_objective"))
            for data in lookup.values()
            if np.isfinite(data.get("best_objective", np.inf))
        ]
        ceiling = max(finite_feasible or finite_any or [0.0])
        margin = max(abs(ceiling), 1.0)
        matrix = np.asarray(
            [
                [
                    self._feasibility_first_merit(lookup[(name, index)], ceiling, margin)
    ...
    def _pairwise_statistics(
        self, rows: list[dict], tables: Path
        ...
        for record, value in zip(raw, corrected):
            record["holm_p_value"] = float(value)
            record["significant_0_05"] = bool(value < 0.05)
        pairwise = pd.DataFrame(raw)
        rank_matrix = np.asarray([rankdata(row, method="average") for row in matrix], dtype=float)
        rankings = pd.DataFrame(
            {"algorithm": algorithms, "average_rank": np.mean(rank_matrix, axis=0)}
    ...
    def _effect_size_plot(self, pairwise: pd.DataFrame, path: Path) -> str:
        if pairwise.empty:
            raise ValueError("Pairwise effect sizes are unavailable")
        fig, ax = plt.subplots(figsize=(8.0, max(4.8, len(pairwise) * 0.45)))
        positions = np.arange(len(pairwise))
        ax.barh(positions, pairwise["cliffs_delta"])
    ...
    def _ranking_plot(self, rankings: pd.DataFrame, path: Path) -> str:
        if rankings.empty:
            raise ValueError("Rankings are unavailable")
        fig, ax = plt.subplots(figsize=(8.0, max(4.8, len(rankings) * 0.42)))
        positions = np.arange(len(rankings))
        ax.barh(positions, rankings["average_rank"])
        ax.set_yticks(positions)
        ax.set_yticklabels(rankings["algorithm"])
        ax.invert_yaxis()
        ax.set_xlabel("Average feasibility-first rank (lower is better)")
        ax.set_title("Algorithm ranking across paired runs")
        ax.grid(True, axis="x", alpha=0.35)
        return self._save_figure(fig, path)
    ...
    def _scenario_matrix(self, rows: list[dict], field: str) -> tuple[list[str], np.ndarray]:
        labels, matrix = [], []
        for row in rows:
            state = (self._load_result(row).get("metadata", {}) or {}).get(
                "solution_state", {}
            ) or {}
            ...
            for scenario in scenarios:
                ...
                values.append(float(value))
            if values:
                labels.append(f"{row['algorithm']} r{int(row['run_index']) + 1}")
    ...
    def _scenario_heatmap(self, rows: list[dict], path: Path, field: str) -> str:
        ...
        fig, ax = plt.subplots(
            figsize=(max(8.0, matrix.shape[1] * 0.22), max(4.5, len(labels) * 0.32))
        )
        image = ax.imshow(matrix, aspect="auto")
        ax.set_yticks(np.arange(len(labels)))
        ax.set_yticklabels(labels)
        ax.set_xlabel("Scenario index")
        ax.set_title("Scenario feasibility" if field == "feasible" else "Scenario active-loss map")
    ...
    def export(
        ...
        *,
        ...
        if not horizons:
            raise ValueError("The experiment has no completed run evidence")
        if evaluation_horizon is None:
            ...
            evaluation_horizon = (
                int(revisions[-1]["evaluation_target"]) if revisions else int(horizons[-1])
            )
        horizon_status = self.database.experiment_horizon_status(
            experiment_id, int(evaluation_horizon)
        )
        # Never reuse/overwrite a resumable portfolio manifest that was built from a different
        # FE horizon or evidence revision. Place the new export in a deterministic child folder
        # so earlier publication evidence remains reproducible.
        revision = horizon_status.get("revision") or {}
        evidence_identity = {
            "objective_function_evaluations": int(evaluation_horizon),
            "revision_id": str(revision.get("id", "legacy-original")),
            "expected_runs": int(horizon_status.get("expected_count", 0)),
            "available_runs": int(horizon_status.get("available_count", 0)),
            "complete": bool(horizon_status.get("complete")),
        }
        previous_identity = dict(manifest.get("evidence_horizon", {}) or {})
        comparable_keys = tuple(evidence_identity)
        if previous_identity and any(
            previous_identity.get(key) != evidence_identity.get(key) for key in comparable_keys
        ):
            revision_tag = revision.get("revision_number", "legacy")
            out = (
                out
                / f"horizon_{int(evaluation_horizon)}_FE_revision_{revision_tag}_runs_{int(horizon_status.get('available_count', 0))}"
            )
            figures = out / "figures"
            tables = out / "tables"
            raw = out / "raw_results"
        ...
        all_rows = list(horizon_status.get("rows", []))
        if not all_rows:
            raise ValueError(
                f"The experiment has no completed runs at {int(evaluation_horizon):,} FE"
            )
        if horizon_status.get("publication_eligible") and not horizon_status.get("complete"):
            raise ValueError(
                f"Publication export is blocked because the {int(evaluation_horizon):,}-FE primary revision is incomplete "
                f"({horizon_status.get('available_count', 0)}/{horizon_status.get('expected_count', 0)} paired runs)."
            )
        verified_rows = [
        ...
        manifest["evidence_horizon"] = {
            **evidence_identity,
            "publication_eligible": bool(horizon_status.get("publication_eligible")),
        ...
        tasks = list(
            dict.fromkeys(
                requested + ["portfolio_tables", "portfolio_metadata", "reproducibility_bundle"]
            )
        )
        completed_count = 0
        ...
        for index, key in enumerate(tasks):
            ...
            try:
                ...
                if key == "objective_convergence":
                ...
                elif key == "constraint_convergence":
                    path = self._single_convergence(
                        row,
                ...
                elif key == "constraint_decomposition":
                    path = self._constraint_decomposition(row, target)
                elif key in {"voltage_profile", "best_validated_voltage_profile"}:
                    path = self._voltage_profile(row, target)
                elif key == "voltage_heatmap":
                    sc = self._solution_scenario(row)
                    path = self._heatmap(
                        np.asarray(sc.get("vm_pu", [])),
                        sc.get("bus_numbers", []),
                        f"Bus-voltage heatmap — {row['algorithm']}",
                        "Bus",
                        target,
                    )
                elif key == "branch_loading":
                    path = self._branch_loading(row, target, False)
                elif key in {"branch_loading_heatmap", "best_validated_branch_heatmap"}:
                    path = self._branch_loading(row, target, True)
                elif key == "generator_reactive_power":
                    sc = self._solution_scenario(row)
                    path = self._heatmap(
                        np.asarray(sc.get("qg_mvar", [])),
                        sc.get("generator_bus", []),
                        f"Generator reactive power — {row['algorithm']}",
                        "Generator bus",
                        target,
                    )
                    ...
                    raise ValueError(
                        "Critical-difference evidence must be generated from at least two separate benchmark cases/formulations in the campaign portfolio"
                    )
