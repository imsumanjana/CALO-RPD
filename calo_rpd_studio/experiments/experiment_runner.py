...
def build_scenarios(config, seed, case=None):
    ...
    if settings.mode == "deterministic":
        ...
        scenarios = generate_load_scenarios(
            ScenarioGeneratorConfig(settings.count, settings.active_load_std, settings.reactive_load_std),
            seed,
        )
    elif settings.mode == "renewable_uncertainty":
        if settings.renewable_bus <= 0 or settings.renewable_rated_mw <= 0:
            raise ValueError("Renewable uncertainty requires a valid bus number and positive rated power")
        if case is not None and int(settings.renewable_bus) not in case.bus_index_map():
            raise ValueError(f"Renewable uncertainty bus {settings.renewable_bus} does not exist in {config.case_name}")
        ...
        if case is not None and any(index >= case.n_gen for index in settings.generator_outages):
            raise ValueError("At least one generator-contingency index is outside the selected case")
    ...
    for item, weight in zip(scenarios, weights, strict=True):
        ...
        if case is not None:
            ...
            if transformed.n_bus != case.n_bus or transformed.n_branch != case.n_branch or transformed.n_gen != case.n_gen:
...
def run_single(config,algorithm,run_index,seeds,progress_callback=None,cancel_callback=None):
    ...
    scenario_payload = {
        ...
        "transformed_case_checksums": [str(item.apply(problem.case).checksum()) for item in scenarios],
    }
    encoded = json.dumps(scenario_payload, sort_keys=True, separators=(",", ":"), allow_nan=False).encode("utf-8")
