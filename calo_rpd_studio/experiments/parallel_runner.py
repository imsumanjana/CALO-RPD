...
def _worker(args):
    config, algo, ri, seeds = args
    return run_single(config, algo, ri, seeds)
    ...
    with ProcessPoolExecutor(max_workers=max(1, config.parallel_workers)) as ex:
        ...
        for f in as_completed(fut):
            ...
            try:
                ...
                failed.append(failed_run_from_exception(algo, ri, s, exc))
    done.sort(key=lambda x: (x.run_index, config.algorithms.index(x.algorithm)))
    failed.sort(
        key=lambda x: (
            x.run_index,
            config.algorithms.index(x.algorithm) if x.algorithm in config.algorithms else 999,
        )
    )
    return done, failed
