...
def voltage_profile_figure(bus_numbers, vm):
    ...
    ax.plot(bus_numbers, vm, marker="o", label="Voltage magnitude")
...
def branch_loading_figure(branch_numbers, loading):
    fig, ax = plt.subplots(figsize=(7.2, 4.6))
    ax.bar(branch_numbers, loading)
    ax.axhline(100, linestyle="--", label="Thermal limit")
    ax.set_title("Branch loading")
