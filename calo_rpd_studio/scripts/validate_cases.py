...
def main():
    ...
    a = p.parse_args()
    case = CaseLoader.load(a.case)
    report = validate_case(case)
    print(f"Case structure valid: {report.valid}")
    pf = run_ac_power_flow(case)
    print(
        f"Power flow converged: {pf.converged}; loss={pf.total_loss_mw:.10g} MW; Q-limit rounds={pf.q_limit_rounds}"
    )
    cross = validate_against_pypower(case, pf)
    print(cross)
    return 0 if report.valid and pf.converged and (not cross.available or cross.passed) else 1
