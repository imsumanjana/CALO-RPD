...
class PortfolioManagerPanel(ScrollablePage):
    ...
    def __init__(self, state, parent=None) -> None:
        ...
        self.kind.addItem("Overall repeated experiment portfolio", PortfolioKind.OVERALL_EXPERIMENT.value)
        ...
        self.custom_runs = QSpinBox(); self.custom_runs.setRange(1, 1000); self.custom_runs.setValue(30)
        ...
        self.preset.addItem("Article 2 — CALO deterministic", ArticlePreset.CALO_DETERMINISTIC.value)
        self.preset.addItem("Article 3 — CALO robust", ArticlePreset.CALO_ROBUST.value)
        self.preset.addItem("Article 4 — experience/accelerator", ArticlePreset.CALO_TRANSFER_ACCELERATOR.value)
        self.storage = QComboBox()
        self.storage.addItem("Minimal diagnostic", StorageProfile.MINIMAL.value)
        self.storage.addItem("Full single-run article diagnostics", StorageProfile.FULL_SINGLE_RUN.value)
        self.storage.addItem("Repeated-run statistical evidence", StorageProfile.REPEATED_STATISTICS.value)
        ...
        select_row.addWidget(select_defaults); select_row.addWidget(clear); select_row.addStretch(1)
        ...
        self.require_validation = QCheckBox("Require independent validation for publication-facing outputs")
        self.reuse = QCheckBox("Reuse exact compatible completed results using scientific fingerprints")
        self.resume = QCheckBox("Enable campaign and job resume")
        self.require_validation.setChecked(True); self.reuse.setChecked(True); self.resume.setChecked(True)
        ...
        self.plan_summary = QLabel(); self.plan_summary.setWordWrap(True)
        self.plan_detail = QLabel(); self.plan_detail.setWordWrap(True); self.plan_detail.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
        plan_layout.addWidget(self.plan_summary)
        plan_layout.addWidget(self.plan_detail)
        buttons = QHBoxLayout()
        preview = QPushButton("Preview required work")
        apply_button = QPushButton("Apply portfolio plan"); apply_button.setObjectName("PrimaryButton")
        preview.clicked.connect(self.refresh_plan)
        apply_button.clicked.connect(self.apply)
        buttons.addWidget(preview); buttons.addWidget(apply_button); buttons.addStretch(1)
    ...
    def _selected_outputs(self) -> list[str]:
        return [key for key, item in self._items.items() if item.checkState(0) == Qt.CheckState.Checked]
    ...
    def _set_outputs(self, keys: list[str]) -> None:
        ...
        try:
            ...
            for key, item in self._items.items():
                item.setCheckState(0, Qt.CheckState.Checked if key in selected else Qt.CheckState.Unchecked)
        finally:
    ...
    def _controls_changed(self, *_args) -> None:
        ...
        if kind is PortfolioKind.SINGLE_RUN:
            ...
            self.storage.setCurrentIndex(self.storage.findData(StorageProfile.FULL_SINGLE_RUN.value))
        else:
            self.profile.setEnabled(True)
            self.custom_runs.setEnabled(str(self.profile.currentData()) == EvidenceProfile.CUSTOM.value)
        self.refresh_plan()
    ...
    def _build_config(self) -> PortfolioConfig:
        return PortfolioConfig(
            ...
            name=("Single-run diagnostic portfolio" if str(self.kind.currentData()) == PortfolioKind.SINGLE_RUN.value else "Overall experiment portfolio"),
        )
    ...
    def refresh(self) -> None:
        ...
        self.profile.setCurrentIndex(max(0, self.profile.findData(portfolio.evidence_profile.value)))
    ...
    def apply(self) -> None:
        try:
            ...
            if not [key for key in portfolio.requested_outputs if key not in plan.disabled_outputs]:
                raise ValueError("None of the selected outputs can be generated from the current formulation.")
            ...
            self.state.config.checkpoint_interval_evaluations = int(portfolio.checkpoint_interval_evaluations)
            fingerprint = stable_sha256({"portfolio": portfolio.to_dict(), "plan": asdict(plan)})
            portfolio_id = self.state.database.create_portfolio(portfolio.name, portfolio.to_dict(), asdict(plan), fingerprint)
