...
| G4 | Hardened static/reproducibility inputs; runtime proof pending | CPU/CUDA profiles, local-only noVNC/VNC, unprivileged UID/GID 10001, dropped capabilities, no-new-privileges, read-only root filesystem, bounded `/tmp`, persistent `/data`, shared-volume device leases, one explicitly selected NVIDIA device, a 24 GiB default host-RAM ceiling, health check and runbook exist. Generated policy/checkpoint/lineage/user files are excluded from the context. The digest-pinned base, dated Debian snapshot and hash-complete CPU/CUDA locks remain enforced. Builds accept full commit/clean declarations, retain deterministic immutable identity plus OCI metadata, copy the license and run `pip check`; runtime evidence prefers live Git and cannot use a declaration to bypass a dirty checkout. CI loads exact-commit CPU/CUDA tags, retains build/image metadata, CycloneDX and full vulnerability reports, gates fixable high/critical findings, and the physical lane retains durable case30/case57/resource/one-hour-soak/cancellation artifacts. Focused contracts pass and BuildKit check is warning-free. Docker Desktop, Compose and Buildx are available; source-bound builds, generated provenance/SBOM/scans, runtime smoke, cross-container lease execution, WSL2 GPU passthrough and GUI proof remain open. |
