...
| G9 | In progress — production optimizer, qualification receipt and counted development environment locally verified; scientific qualification pending | A–F mechanics remain green and F remains experimental/off by default. `TSHCALOOptimizer` requires a separately qualified, explicitly activated immutable ensemble before any power-system evaluation; consumes counted topology contexts, shielded per-learner actions and the canonical transition; preserves exact accounting/resume; and blocks or explicitly relabels fallback. TSH activation requires an integrity-checked development-only qualification receipt binding the exact policy, protocol, seeds, evidence-artifact reference and OOD calibration. The independent development environment now binds the training/environment/problem identities, rejects protected loaded cases, produces counted topology observations without hidden power-flow solves, executes raw member actions through the same transition, masks unavailable E, rejects F before evaluation and resumes exactly without experiment/registry/activation authority. Solver exceptions poison the instance and retain incomplete-accounting provenance instead of permitting a silent reset. Eight dedicated environment tests, a 29-test adjacent focus and the 555-pass active tree pass. These checks establish mechanics and receipt integrity, not evidence sufficiency or performance. Safe-80 trainer admission, fresh ensemble members, formal qualification, target-CUDA parity/pressure traces, counted Jacobian availability for E, ablation benefit and Change-F promotion remain absent. |
