...
| G6 | Implemented; Linux container rendering retained, packaged-lane execution pending | Normal experiment UI exposes two compute choices and no device percentages/batches/schema controls; policy UI hides No-AI/unqualified and routing internals; Dashboard readiness exposes available memory, admission status and recoverable queue progress instead of utilization/worker engineering. A rendered-widget contract checks Dashboard, Experiment Manager, Portfolio Manager, CALO Intelligence and Benchmark & Evidence for venue/development/backend/schema/XPU/Safe-80/utilization language. The complete Windows/offscreen GUI suite passes locally (33 tests) and produced a validated 1440x900 dashboard PNG. Corrected clean Linux CPU image `cpu-31a4713` rendered and restored the 1600x1000 Dashboard through xcb with identical pre/post-restart evidence. CI persists the corresponding Linux rendering plus accessibility evidence; its first packaged execution and browser interaction remain pending. |
...
| G9 | In progress — counted E development complete; valid negative screen retained; old candidate rejected | A–F mechanics remain green and F remains experimental/off by default. Runtime `tsh-calo-v1.1.0-counted-physics-candidate` and training ABI v4 now retain the final counted Newton Jacobian, analytic relaxed-control sensitivity and active-constraint projection only when immutable Change E is enabled; production/training masks fail closed otherwise, records linear-algebra cost, preserves exact FE/scenario-call accounting, and never declares feasibility. A real case30 invariant caught and fixed post-snap trust-radius escape. The TSH/counted-ORPD unit family passes 136 tests. Historical candidate v2 completed five CUDA members and a valid negative 40-cell screen, remains unqualified/inactive, and is not reinterpreted under v1.1/v4. Its case57 interval crossed zero and Holm-adjusted `p=0.052734375` missed the frozen threshold; no receipt, registration or activation exists. A new frozen candidate/design, target CPU/CUDA parity, accepted A–E incremental-value/cost evidence and every qualification criterion remain absent; protected cases stay closed. |
| G10 | Partially complete; mathematical-reference implementation retained | Statistical corrections, honest claim boundaries, power-aware planning and a frozen-design preregistration protocol exist in `SCIENTIFIC_VALIDATION_PROTOCOL.md`. The runtime-enumerated 22-method campaign defaults to 98 initiated paired runs for 21 CALO-versus-comparator tests at effect 0.50, 95% power, Holm family control and 10% failures; pilot/simulation designs require an evidence SHA. case30/57 are validation replays, case118/300 are protected tests. Source-traceable L-SHADE 1.0.1 and pinned pycma 4.4.4 have deterministic snapshots. Official PGLib-OPF v23.07 case14 typical/API/SAD validation assets have exact source/license provenance and a restricted non-executing parser; the strict reviewed-profile boundary never infers controls. Clean commit `07f9476` adds disclosed SciPy SLSQP local continuous-relaxation and bounded exhaustive all-discrete adapters, explicit no-bound/no-gap semantics, separate original-lattice reevaluation, call/termination/derivative accounting, independent PYPOWER checks, protected-case refusal and new-file-only clean-source evidence. Eleven focused tests, a 37-test combined adjacent regression set and the complete active suite (`629 passed, 63 skipped`) pass. Real bounded case30 probes retained honest infeasible/iteration-limit outcomes; they are interface validation, not solver superiority or task-optimum evidence. Human-reviewed external ORPD profiles, full multistart/reference campaign execution, approved-architecture ablations, protected campaign execution and remaining final qualification still remain. |
...
- Independent trainer Safe-80 admission: current training ABI
  `tsh-calo-training-v4-counted-physics-safe80-receipts` (historical v3 artifacts remain immutable and non-native to v4); hash-bound rollout/population/topology/scenario envelope;
