...
| G9 | In progress — A–F mechanics and candidate lifecycle locally verified; scientific evidence pending | A–F component evidence remains green and F remains double-gated, experimental and off by default. The immutable candidate boundary now exports only explicitly unqualified TSH-CALO artifacts with exact algorithm/policy/state/action/training ABIs, feature flags, independent-training design/seed hashes, source commit and development-case identities. Protected holdouts are rejected before export. The algorithm-aware registry preserves frozen CALO compatibility, refuses TSH self-qualification and unqualified activation/consumption, verifies SHA-256 on activation/load, and binds identity plus scientific provenance under `TSH-CALO` rather than `CALO`. Seven lifecycle tests and a 33-test focused compatibility/independence set pass; the active tree is 506 passed with 63 skips. No trained candidate, qualification result, activation, ablation benefit or Change-F promotion exists. |
