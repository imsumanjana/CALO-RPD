...
| G9 | In progress — production optimizer, qualification receipt and frozen-plan counted training campaign locally verified; scientific qualification pending | A–F mechanics remain green and F remains experimental/off by default. `TSHCALOOptimizer` requires a separately qualified, explicitly activated immutable ensemble before any power-system evaluation and preserves exact accounting/resume/fallback identity. TSH activation requires an integrity-checked qualification receipt. Training ABI v3 provides counted development episodes, canonical rewards, Safe-80 admission, exact-device/session resume, immutable device provenance and terminal receipts. The explicit campaign freezes source/design/execution/seed/curriculum identities, requires independent member runs, checkpoints every transition, blocks same-identity retry after counted failure, and emits only unqualified members/ensemble. Its command requires the exact source commit and clean tracked tree and owns no experiment or lifecycle authority. Six campaign cases, a 61-test adjacent focus and the 575-pass active tree pass. These checks establish toy-fixture mechanics, not evidence sufficiency or performance. No IEEE plan or fresh real member exists; formal qualification, physical target-CUDA parity/pressure traces, counted Jacobian availability for E, ablation benefit and Change-F promotion remain absent. |
...
- Frozen-plan independent training campaign: exact source/design/execution/seed/curriculum hashes;
  ...
  resume plus status path/SHA verification; irreversible same-identity failure status; and explicit
  unqualified-only member/ensemble outputs. **6 passed** dedicated campaign cases and **61 passed**
  across campaign, session, trainer, environment, resources, lifecycle, inference and optimizer. The
  active tree is **575 passed, 63 skipped**; Ruff lint/format passes across **403 Python files** and
  the generated schema is current. Only toy fixtures executed; no IEEE training or scientific result
  exists.
