...
| Six-step inline Study Setup separated from the Dashboard | Implemented; Case, Formulation, and Scenarios use shared-state specialist panel classes inline; redundant Algorithms step removed | Prior Windows interaction evidence accepted for the shell; fresh Phase 6 source-bound validation required for the inline integration; human acceptance remains separate |
