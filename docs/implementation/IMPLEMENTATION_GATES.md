...
| G9 | In progress — non-tuning development complete through A–E evidence/device gates; execution pending | A–F mechanics remain green and F remains experimental/off by default. Runtime `tsh-calo-v1.1.0-counted-physics-candidate` and training ABI v4 retain counted E with exact FE/scenario-call accounting and fail-closed masks. Commits `ae7b304` and `e77431e` add the previously missing repository-owned frozen A–E paired-ablation producer, strict formal-evidence verification and an immutable-candidate deterministic CPU/CUDA policy-equivalence validator with dedicated-VRAM proof. Both are development-only and have no training/lifecycle authority; short focused gates pass 33 and 11 tests respectively. Historical candidate v2 and its valid negative 40-cell screen remain immutable, unqualified/inactive and incompatible with counted v1.1/v4; no threshold is weakened or reinterpreted. A fresh preregistered counted-v4 A–E candidate, its physical equivalence result, accepted component evidence and qualification result remain absent; protected cases stay closed. |
