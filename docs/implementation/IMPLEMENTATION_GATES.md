...
| G9 | In progress — A–D locally verified; E next | A–C evidence remains green. Change D adds probability-space ensemble disagreement, a frozen development-reference OOD signature/calibration, exact-resumable sliding-window group/context bandit residuals, a declared neural/rule/bandit/exploration mixture, strict operator/FE/lattice shielding, deterministic traces and explicit block-or-frozen-baseline fallback identity. Six focused Change-D tests pass; the A–D focus is 28 passed and the complete active tree is 480 passed with 63 skips. E–F remain unimplemented; F remains experimental and disabled by default. No trained TSH-CALO policy, ablation result, qualification or activation exists. |
