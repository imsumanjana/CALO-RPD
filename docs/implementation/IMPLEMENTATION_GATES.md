...
| G9 | In progress — A–C locally verified; D next | A/B evidence remains green. Change C adds a topology-plus-aggregate actor critic with one global regime head, three physical control-group operator/parameter heads, four learner-context residual heads, strict group/operator masks, explicit unavailable-group sentinels, bounded controls and independent replayable per-learner sampling on the active tensor device. Eight focused Change-C tests pass, including the available bounded CPU/CUDA forward-agreement check; the complete active tree passes 474 tests with 63 skips. D–F remain unimplemented; F remains experimental and disabled by default. No trained TSH-CALO policy, ablation result, qualification or activation exists. |
