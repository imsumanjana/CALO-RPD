...
| G9 | In progress — A–F, lifecycle, independent PPO and shielded inference cores locally verified; optimizer integration pending | A–F evidence remains green and F remains experimental/off by default. Immutable ensemble lifecycle and qualified/activated inference checks remain green. ORPD now offers an explicit `evaluate_with_context` path that returns ephemeral power-flow objects from the same counted scenario evaluations; ordinary `evaluate` remains value-equivalent, solver context is excluded from serializable result metadata, the base/weighted converged solve is selected without a new PF call, and missing convergence fails closed. Three dedicated context tests, a 37-test topology/repair/transition/frozen-CALO focus and the 523-pass active tree pass. The new optimizer transition, real candidates, target-CUDA traces, qualification, ablation benefit and Change-F promotion remain absent. |
