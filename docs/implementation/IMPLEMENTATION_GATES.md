...
| G9 | In progress — A–F mechanics, candidate lifecycle and independent PPO core locally verified; scientific evidence pending | A–F component evidence remains green and F remains double-gated, experimental and off by default. The immutable candidate boundary exports only explicitly unqualified, exact-ABI artifacts and rejects protected holdouts. The algorithm-aware registry preserves frozen CALO compatibility and refuses TSH self-qualification or unqualified activation/consumption. The independent TSH trainer has its own configuration/design hash, accepts already-built topology rollout states, performs masked hierarchical PPO updates, selects CUDA first when available, restores model/optimizer/local RNG state by required checksum and refuses scientific-design drift. It imports no experiment workflow or registry authority and can only export an unqualified candidate after an update. Seven trainer tests, a 22-test trainer/lifecycle/action focus and the 513-pass active tree pass. No real candidate, target-CUDA training trace, qualification, activation, ablation benefit or Change-F promotion exists. |
