...
| G9 | In progress — A–E locally verified; F mechanics locally verified but evidence-gated | A–E evidence remains green. Change F is implemented only as a separately double-gated experimental population schedule: both `enabled` and `experimental_mode` must be true, while repository defaults keep it off. Its preregistered design hash binds feasibility, archive-coverage, diversity, remaining-budget, spacing and contraction bounds; selection is deterministic and feasibility-first, adds no FE, and resumes exactly. Nine focused Change-F tests pass; cumulative A–F mechanics are 47 passed. The active tree excluding the deliberately stale v6.9 release-integrity file is 499 passed with 63 skips; the complete tree is 502 passed, 63 skipped, 2 expected release-integrity failures. No Change-F benefit, acceptable-cost evidence, trained TSH-CALO policy, qualification or activation exists. |
