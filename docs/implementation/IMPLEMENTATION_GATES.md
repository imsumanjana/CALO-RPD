...
| G9 | In progress — A/B locally verified; C next | Change A routes deployed CALO and independent PPO rollouts through `algorithms/calo/transition_kernel.py`; its frozen parity evidence remains green. Change B adds a distinct TSH-CALO candidate algorithm/state/action ABI, strict aggregate-plus-topology state, already-computed-power-flow graph construction, and dependency-light message passing with graph/control-group pooling. Five Change-B tests cover ABI separation, F-off defaults, strict state validation, no hidden solver call, deterministic permutation invariance and topology sensitivity; the complete active tree passes 466 tests with 63 skips. C–F remain unimplemented; F remains experimental and disabled by default. No fresh policy, ablation result, or qualification evidence exists yet. |
