...
| G9 | In progress — A–F, lifecycle, independent PPO, shielded inference and runtime-transition mechanics locally verified; optimizer orchestration pending | A–F evidence remains green and F remains experimental/off by default. Immutable ensemble lifecycle and qualified/activated inference checks remain green. The runtime topology builder consumes only ephemeral scenario solves from `evaluate_with_context`, measures declared scenario stress without a second PF call, and selects the base/highest-weight converged reference fail-closed. The versioned TSH candidate kernel consumes independently shielded learner group/operator/parameter actions, preserves forced recovery and counted precision, executes optional physics repair only from supplied counted context, and reserves a separate precision-memory channel. A 68-test expanded runtime/frozen-parity focus and the 532-pass active tree pass. End-to-end optimizer orchestration, real candidates, target-CUDA traces, qualification, ablation benefit and Change-F promotion remain absent. |
