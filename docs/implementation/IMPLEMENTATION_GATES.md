...
| G9 | In progress — A–E locally verified; F experimental next | A–D evidence remains green. Change E expands only the new TSH-CALO ABI to a seventh operator, masked by default. It accepts caller-supplied linearization retained from an already-counted converged evaluation, masks missing/unconverged/ill-conditioned contexts, performs bounded linear algebra without a power-flow call, snaps declared discrete lattices, never declares feasibility and requires one explicit trusted evaluation within the remaining FE budget. Ten focused Change-E tests pass; cumulative A–E focus is 38 passed and the complete active tree is 490 passed with 63 skips. F remains experimental, unimplemented and disabled by default. No trained TSH-CALO policy, ablation result, qualification or activation exists. |
