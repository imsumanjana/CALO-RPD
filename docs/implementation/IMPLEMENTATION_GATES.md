...
| G9 | In progress — complete unqualified ensemble and qualification harness; ablation/qualification evidence pending | A–F mechanics remain green and F remains experimental/off by default. `TSHCALOOptimizer` still requires a separately qualified, explicitly activated immutable ensemble in ordinary execution. Training ABI v3 and the frozen campaign retain exact source/design/execution/seed/curriculum, counted FE/scenario/transition/update receipts, Safe-80 device provenance and narrow interruption semantics. Real campaign v2 completed five distinct IEEE 30/57 CUDA members and an immutable unqualified ensemble with exact aggregate 100,000 FE/scenario calls. The explicit qualifier now fits development-only OOD calibration, retains paired equal-FE candidate/baseline records and failures, independently validates solutions, reports feasibility/effects/bootstrap CIs/Holm/anytime evidence and cannot register or activate. Screening cannot issue a receipt; formal issuance requires at least 30 pairs per case plus checksum-verified accepted A–E component evidence, and excludes F. Physical target CPU/CUDA parity/pressure, counted Jacobian availability for E, A–E ablation benefit and any qualification remain absent. |
