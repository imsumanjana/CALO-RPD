...
| G4 | Source-bound CPU/CUDA core runtime/security/soak retained; GUI image correction pending rebuild | Hardened CPU/CUDA profiles retain local-only noVNC/VNC, UID/GID 10001, dropped capabilities, no-new-privileges, read-only root, bounded `/tmp`, `/data`, shared-volume leases, one GPU and host-RAM ceiling. Exact clean commit `1f02a94` produced attested CPU/CUDA images, maximum BuildKit provenance/SBOM, metadata, filesystem manifests, CycloneDX and complete Trivy JSON; both privacy audits and zero-fixable-HIGH/CRITICAL gates passed. CPU/CUDA smoke, cross-container lease, case30/case57 parity and resource recovery passed. The suspended 1,203-sample soak remains rejected. Its continuous exact-image rerun completed 3,600 GREEN samples, exit 0, zero safe-stop/protection events, verified 3,602-event chain and maximum UTC gap 1.106 seconds. GUI qualification then correctly rejected the CPU image: Qt xcb could not load because `libxcb-shape.so.0` was absent while noVNC-only health briefly reported healthy during the restart loop. Source now adds `libxcb-shape0`, build-time `libqxcb.so` dependency loading, live Qt PID health and all-child supervision; focused tests/static build checks pass. Rebuild/runtime browser/restart/persistence/cancellation proof, then final-candidate/CI repetition, remain before G4 closes. |
...
| G10 | Partially complete | Statistical corrections, honest claim boundaries, power-aware planning and a frozen-design preregistration protocol exist in `SCIENTIFIC_VALIDATION_PROTOCOL.md`. The runtime-enumerated 22-method campaign defaults to 98 initiated paired runs for 21 CALO-versus-comparator tests at effect 0.50, 95% power, Holm family control and 10% failures; pilot/simulation designs require an evidence SHA. Immutable campaign design is hashed while runtime status remains updateable. case30/57 are validation replays, case118/300 are protected tests, and a confirmatory plan cannot omit all protected tests or relabel one. Source-traceable L-SHADE 1.0.1 and official pinned pycma 4.4.4 baselines have deterministic snapshots and focused formulation tests. The official PGLib-OPF v23.07 case14 typical/API/SAD validation assets are now retained unmodified with CC-BY-4.0 attribution, exact release commit, code-rooted manifest hashes and source hashes. A non-executing restricted MATPOWER parser verifies both hashes, role and variant and records provenance; it never infers ORPD controls. A separate strict reviewed-profile boundary binds explicit voltage/tap/shunt controls to the exact source and physical checksums, and protected roles require a second explicit test-only authorization. No human-reviewed external ORPD profile is fabricated or populated. Mathematical-solver comparisons, reviewed external profiles, approved-architecture ablations, protected campaign execution and remaining physical qualification still remain. |
