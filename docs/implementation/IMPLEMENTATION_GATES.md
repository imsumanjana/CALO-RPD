...
| G9 | In progress — A–F, lifecycle, independent PPO and shielded inference cores locally verified; scientific evidence pending | A–F evidence remains green and F remains double-gated, experimental and off by default. Independently trained members can now be assembled into one immutable unqualified ensemble while preserving every member SHA/provenance; a single member cannot be activated. Qualified TSH binding additionally requires explicit activation. The inference core verifies the ensemble SHA, exact ABIs, feature flags/member provenance and frozen OOD-calibration hash; performs CUDA-first Safe-80 admission from currently free VRAM with CPU fallback only when the admitted estimate cannot fit or an actual CUDA allocation fails; runs ensemble disagreement plus the shield; and returns execute, block or explicitly relabelled frozen-baseline fallback with device/intervention provenance. Six inference tests and a 35-test inference/lifecycle/shield/trainer/action focus pass; the active tree is 520 passed with 63 skips. Actual optimizer integration, real candidates, target-CUDA traces, qualification, ablation benefit and Change-F promotion remain absent. |
