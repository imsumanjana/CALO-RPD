...
| G9 | In progress — A parity refactor first | Implementation is authorized only in the documented order. Change A must prove seeded zero-difference parity before B–F. F remains independently flagged and disabled by default. No fresh policy, ablation result, or qualification evidence exists yet. |
