...
| G9 | In progress — production optimizer, qualification receipt, counted development environment and Safe-80 trainer admission locally verified; scientific qualification pending | A–F mechanics remain green and F remains experimental/off by default. `TSHCALOOptimizer` requires a separately qualified, explicitly activated immutable ensemble before any power-system evaluation; consumes counted topology contexts, shielded per-learner actions and the canonical transition; preserves exact accounting/resume; and blocks or explicitly relabels fallback. TSH activation requires an integrity-checked development-only qualification receipt binding the exact policy, protocol, seeds, evidence-artifact reference and OOD calibration. The independent development environment binds training/environment/problem identities, rejects protected loaded cases, produces counted topology observations, executes raw member actions through the canonical transition, masks unavailable E, rejects F before evaluation and poisons solver-failed instances. Training ABI v2 adds a hash-bound rollout/topology envelope, a deterministic working-set estimate, CUDA-first admission against 80% of current free VRAM, explicit CPU admission against 80% of current available RAM, cross-process/local CUDA ownership, allocator ceiling, exact-device resume and immutable candidate device provenance. Seven resource tests and a 49-test adjacent focus pass. These checks establish mechanics, not evidence sufficiency or performance. Fresh ensemble members, formal qualification, physical target-CUDA parity/pressure traces, counted Jacobian availability for E, ablation benefit and Change-F promotion remain absent. |
...
- Independent trainer Safe-80 admission: training ABI
  `tsh-calo-training-v2-counted-safe80`; hash-bound rollout/population/topology/scenario envelope;
  ...
  semantics; immutable candidate provenance validation; and v1 incompatibility classification.
