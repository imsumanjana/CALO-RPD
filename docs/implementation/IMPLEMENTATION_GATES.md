...
| G9 | In progress — A locally verified; B next | Change A now routes deployed CALO and the independent PPO rollout environment through `algorithms/calo/transition_kernel.py` for candidate assignment/operator execution, explicit candidate-batch accounting, personal/archive/credit updates, environmental selection, reward and next diagnostics. Frozen evidence passes: native runtime/training parity (1), CALO/continuation focus (45), all 22 seeded optimizer snapshots/exact budgets (22), dedicated kernel invariants (8), and the complete active tree (461 passed, 63 skipped). B–F remain unimplemented; F remains independently flagged and disabled by default. No fresh policy, ablation result, or qualification evidence exists yet. |
