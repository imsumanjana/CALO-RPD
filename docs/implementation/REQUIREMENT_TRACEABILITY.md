...
| Independent TSH-CALO policy training | PPO, canonical-reward collector, counted development environment, fresh-member session and Safe-80 trainer admission locally verified; physical execution pending | `tsh_calo_training.py`, `tsh_calo_training_environment.py`, `tsh_calo_training_resources.py`, `tsh_calo_training_session.py`, `tsh_calo_training_receipt.py`; v3 counted/Safe-80/receipt ABI; independent training/environment/session design hashes; unique session IDs; loaded-case checksum/formulation binding; protected identity exclusion; counted full batches and topology state; canonical transition rewards; exact FE/scenario/update/reward-sequence accounting; joint trusted exact resume; solver-failure poisoning and failed-session checkpoint refusal; hash-bound resource envelope; deterministic working-set estimate; CUDA-first current-free-VRAM and governed current-available-RAM admission; device leases/allocator ceiling; exact-device resume; immutable candidate device provenance and mandatory counted terminal-episode receipts; E mask; pre-solve F rejection; explicit unqualified-only export; no experiment/registry/qualification/activation/inference authority | Execute fresh independent ensemble-member training and retain physical CPU/CUDA provenance; no trained candidate exists yet |
...
- Active development suite after the counted fresh-member session boundary, excluding the
  deliberately stale v6.9 release-integrity file: **568 passed, 63 skipped**. The preceding complete-tree checkpoint
...
development environment and fresh-member session orchestration, shielded inference, versioned
runtime transition and policy-gated optimizer mechanics have passed their local correctness gates.
Change F has not earned promotion and remains off. The next legal step is executing fresh
development-only ensemble-member training from a frozen declared design, then formal qualification
against preregistered evidence. No protected case may enter training or tuning. Paired-seed component
ablations must retain the disabled baseline and may not weaken preregistered acceptance criteria.
