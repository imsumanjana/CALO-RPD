...
| Duplicate-free ribbon ownership | `RIBBON_CATEGORY_ORDER`; `COMMAND_SPECS`; `RibbonBar`; Phase 6 command, GUI, and offscreen contracts | Workspace and every `workspace.*` duplicate are absent. Each visible command label is unique. Algorithms owns CALO, Experiment owns study/model/scenario/portfolio, Compute owns Live and Statistics, Results owns review/validation/benchmark/export, Policies owns training, and Help owns Settings. Stable workspaces and prerequisite gates remain unchanged. Implemented; fresh owner Phase 6 validation pending. |
