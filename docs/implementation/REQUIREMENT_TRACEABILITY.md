...
| Independent TSH-CALO policy training | PPO, canonical-reward collector, counted development environment/session, frozen-plan campaign and Safe-80 admission locally verified; physical execution pending | `tsh_calo_training.py`, `tsh_calo_training_environment.py`, `tsh_calo_training_resources.py`, `tsh_calo_training_session.py`, `tsh_calo_training_receipt.py`, `tsh_calo_training_campaign.py`, `scripts/train_tsh_calo.py`; v3 counted/Safe-80/receipt ABI; exact source/scientific/execution/seed/curriculum hashes; independent member seeds/run IDs/artifacts; unique sessions; loaded-case checksum/formulation and protected-identity guards; canonical rewards and exact FE/scenario/update/reward accounting; per-transition authenticated exact resume plus status SHA/path binding; poison/failure retention and same-identity retry refusal; CUDA-first current-free-VRAM and governed current-available-RAM admission; immutable device provenance; E mask and pre-solve F rejection; explicit unqualified-only member/ensemble output; clean-source command; no experiment/registry/qualification/activation/inference authority | Freeze an IEEE 30/57 campaign plan, execute fresh independent ensemble members and retain physical CPU/CUDA provenance; no real trained candidate exists yet |
...
- Active development suite after the frozen-plan fresh-member campaign boundary, excluding the
  deliberately stale v6.9 release-integrity file: **575 passed, 63 skipped**. The preceding complete-tree checkpoint
...
Changes A–E, disabled Change-F mechanics, immutable ensemble lifecycle, independent PPO, counted
development environment/session and frozen-plan campaign execution, shielded inference, versioned
runtime transition and policy-gated optimizer mechanics have passed their local correctness gates.
Change F has not earned promotion and remains off. The next legal step is freezing the actual IEEE
30/57 member count, seeds, curriculum, FE budgets and PPO controls in a clean-source campaign plan,
then executing it independently. Formal qualification follows only after those immutable candidates
exist. No protected case may enter training or tuning. Paired-seed component ablations must retain
