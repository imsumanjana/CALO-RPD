...
| Six-step inline Study Setup | `StudySetupWorkflow`; inline `PowerSystemPanel`, `ORPDFormulationPanel`, and `RobustScenariosPanel` instances over shared `AppState`; mode-correct algorithm summary | Case, Formulation, Budget/runs, Scenarios, Validate/outputs, and Review/launch stay in one workspace. Workspace shows the retained Portfolio subset; Individual shows the complete submitted stage; no second selector or linked navigation is present. Focused synthetic GUI contracts passed; fresh complete Phase 6 validation required. | Human usability/scientist acceptance remains separate |
...
| Immutable Workspace and individual execution ownership | `experiments/execution_plans.py`; `app/execution_control.py`; `app/workspace_campaign.py`; schema-v2 `results/database.py`; Portfolio/Algorithms/Study panels; focused unit/integration/GUI contracts | Explicit Algorithms Submit persists the only active stage. Workspace binds a non-empty subset and explicit cells; individual binds the full unchanged stage. Canonical hashes, audit receipts, a singleton fenced controller, atomic Workspace pause release, individual retained pause ownership, resume interlock, terminal cancellation, parent plan/cell task identities, and duplicate-admission rejection fail closed. The existing ExperimentManager remains the only numerical executor. CALO/TSH-CALO and policy/training/qualification semantics are unchanged. Implemented; owner Phase 6 schema-v9 validation pending. |
| Truthful Workspace draft/audit status | `ExperimentManagerPanel._invalidate_fairness`; signal-blocked shared-state hydration; `refresh_execution_state`; `test_workspace_execution_ui.py`; validator schema v26 | Completing setup does not bypass fairness. A never-audited draft reports that its fairness pass is required rather than claiming an earlier configuration changed. Programmatic refresh preserves truthful state; a real post-audit edit disables Stage immediately and requires re-audit. Audit receipt, immutable-plan, controller, and Run gates are unchanged. Focused regressions passed; complete source-stable v26 validation pending. |
