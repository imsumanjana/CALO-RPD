...
| Independent TSH-CALO policy training | PPO, counted development environment/session, frozen-plan campaign and Safe-80 admission locally verified; first real attempt retained failed | `tsh_calo_training*.py`, `scripts/train_tsh_calo.py`; v3 counted/Safe-80/receipt ABI; exact source/scientific/execution/seed/curriculum hashes; independent member seeds/run IDs/artifacts; protected-identity guards; canonical rewards and exact FE/scenario/update/reward accounting; authenticated per-transition resume plus status SHA/path; terminal solver/trainer/integrity/design failures; narrowly resumable accounting-complete infrastructure `OSError`; CUDA-first current-free-VRAM admission and immutable device provenance; E/F off; unqualified-only output; no experiment/lifecycle authority. Real v1 attempt retained three complete CUDA members and an authenticated partial fourth before Windows status replacement failed; no ensemble exists | Execute a new clean-source IEEE 30/57 campaign under corrected interruption semantics, then retain physical CPU/CUDA parity evidence; no qualified candidate exists |
...
- Active development suite after infrastructure-interruption classification, excluding the
  deliberately stale v6.9 release-integrity file: **576 passed, 63 skipped**. The preceding complete-tree checkpoint
...
Change F has not earned promotion and remains off. The first frozen IEEE 30/57 attempt is retained
failed after an infrastructure status-write error and its partial candidates cannot be promoted. The
next legal step is a new campaign identity bound to the corrected clean source, with member count,
seeds, curriculum, FE budgets and PPO controls frozen before execution. Formal qualification follows
only after a complete immutable ensemble exists. No protected case may enter training or tuning.
Paired-seed component ablations must retain the disabled baseline and may not weaken preregistered
acceptance criteria.
