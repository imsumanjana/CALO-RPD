...
| Preserve old populated databases through rollback-safe migration | Implemented / fresh proof pending | SQLite schema v2, online pre-migration backup, integrity check, SHA-256 receipt, transactional DDL, future-version rejection, execution-controller migration contracts; `tests/integration/test_database_workflow.py`; `tests/integration/test_workspace_execution_control.py` | Fresh owner Phase 6 validator and CI matrix execution |
...
| Six-step mode-correct experiment setup | `StudySetupWorkflow`; inline `PowerSystemPanel`, `ORPDFormulationPanel`, and `RobustScenariosPanel` instances over shared `AppState`; separate Workspace/Individual prerequisite ledgers; mode-correct algorithm summary | Case, Formulation, Budget/runs, Scenarios, Validate/outputs, and Review/launch stay in one workspace. Workspace Study shows the retained Portfolio subset and derived run count. Individual Experiment shows the complete submitted stage, editable direct run count, and no Portfolio prerequisite; no second selector or linked navigation is present. Current correction awaits complete Phase 6 schema-v28 validation. | Human usability/scientist acceptance remains separate |
...
| Immutable Workspace and individual execution ownership | `experiments/execution_plans.py`; `app/execution_control.py`; `app/workspace_campaign.py`; schema-v2 `results/database.py`; Portfolio/Algorithms/Study panels; focused unit/integration/GUI contracts | Explicit Algorithms Submit persists the only active stage. Workspace binds a non-empty subset and explicit cells; individual binds the full unchanged stage. Canonical hashes, audit receipts, a singleton fenced controller, atomic Workspace pause release, individual retained pause ownership, resume interlock, terminal cancellation, parent plan/cell task identities, and duplicate-admission rejection fail closed. The existing ExperimentManager remains the only numerical executor. CALO/TSH-CALO and policy/training/qualification semantics are unchanged. The pre-correction controller contracts passed schema v27; the current v2 mode separation awaits schema v28. |
...
| Independent direct experiment versus automated Workspace study | `WorkflowManager.individual_completed`; `ExperimentManagerPanel`; `frozen_individual_config_payload`; `result_contracts.py`; `ExperimentWorker`; plan schemas v2; focused unit/GUI/integration/restore contracts | Experiment > Individual experiment owns its setup ledger, repetitions, exact verified reuse, direct audit/result contract, and immutable full-stage plan without consuming Portfolio ID/configuration or Workspace study metadata. Workspace continues to bind its applied Portfolio subset, derived repetitions/outputs/evidence, case matrix, and automated cells to the same ExperimentManager. Controller exclusivity, audit-before-stage-before-run, scientific semantics, and policy gates remain intact. Implemented in source; complete owner Phase 6 schema-v28 validation pending. |
