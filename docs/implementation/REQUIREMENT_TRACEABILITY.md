...
| Guarded physical completed-model deletion | `TrainingModelLibrary.delete_completed_campaign`; `CALOIntelligencePanel._delete_completed_training`; synthetic unit/GUI contracts | Exact confirmed deletion is limited to an unregistered completed child directory. Scan roots, symlinks, incomplete or undiscoverable state, and registered/active policy paths are rejected. Validator does not perform deletion. |
