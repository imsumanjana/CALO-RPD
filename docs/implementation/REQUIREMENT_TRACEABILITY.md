...
| Versioned TSH-CALO candidate lifecycle | Locally verified through optimizer consumption | `tsh_calo_policy_artifact.py`, algorithm-aware `policy_schema.py` and `policy_registry.py`; immutable SHA-256, exact ABI and independent-training provenance; protected-holdout rejection; candidate-only registration; qualified-only explicit activation/binding; policy-gated runtime preflight and fallback | Produce fresh independent members and implement formal qualification/calibration binding; synthetic lifecycle fixtures are not candidate evidence |
...
- Active development suite after policy-gated optimizer integration, excluding the deliberately stale
  v6.9 release-integrity file: **540 passed, 63 skipped**. The preceding complete-tree checkpoint
...
is development-only rollout production for fresh independent ensemble members, followed by formal
qualification that freezes and binds the OOD calibration before activation. Fresh candidates must be
