...
| Single-row Policy Library actions | `CALOIntelligencePanel` action layout and eligibility visibility; Phase 6 static contract | Compare and archive controls are absent. Activation is hidden until scientist selection or legacy qualification makes it eligible, so no disabled `Scientist selection required` placeholder is shown. Import, assess, select, eligible activation, guarded delete, refresh, and archived-record visibility share one row. Scientific and lifecycle semantics are unchanged; validation pending. |
