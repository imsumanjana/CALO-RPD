...
| Complete saved-training path visibility | `_ResponsivePathLabel`; full-width saved-training host; constrained GUI/offscreen contracts | Long filesystem paths wrap anywhere, reserve their calculated full height, remain mouse-selectable, and retain the exact path as a tooltip without covering the next input. Implemented; owner render validation pending. |
