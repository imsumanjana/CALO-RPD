...
| Non-root, read-only, capability-dropped runtime with persistent data volume | Core runtime physically verified; GUI correction pending rebuild | Exact images passed UID 10001, read-only root, capability/no-new-privilege invocation, writable data, schema round-trip and shared `/data/device-leases`; independent CUDA containers proved exclusion then post-release acquisition. GUI attempt exposed missing `libxcb-shape.so.0` and a noVNC-only false-health window; image was rejected and source now requires xcb dependency closure plus live-app PID/all-child supervision | Rebuild and complete browser GUI/restart/persistence/cancellation qualification; repeat on final candidate and CI |
