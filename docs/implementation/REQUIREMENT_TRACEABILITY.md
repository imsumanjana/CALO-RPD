...
| Preserve old populated databases through rollback-safe migration | Implemented / fresh proof pending | SQLite schema v3, online pre-migration backup, integrity check, SHA-256 receipt, transactional DDL, future-version rejection, legacy combined Portfolio/Study preservation, additive Portfolio goal/Study setup tables, and execution-controller migration contracts; `tests/integration/test_database_workflow.py`; `tests/integration/test_workspace_execution_control.py` | Fresh owner Phase 6 schema-v30 validator and CI matrix execution |
...
| Six-step mode-correct experiment setup | `StudySetupWorkflow`; inline `PowerSystemPanel`, `ORPDFormulationPanel`, and `RobustScenariosPanel` instances over shared `AppState`; `AppliedPortfolioGoal`; `StudyRecommendation`; separate Workspace/Individual prerequisite ledgers | Case, Formulation, Budget/runs, Scenarios, Validate/outputs, and Review/launch stay in one workspace. Workspace Study retains the inline step strip. Individual Experiment uses the six existing ribbon options without repeating that strip, shows the complete submitted stage, exposes editable direct values, and has no Portfolio prerequisite. Source implemented; schema-v30 owner validation pending. | Human usability/scientist acceptance remains separate |
...
| Independent direct experiment versus automated Workspace study | `WorkflowManager.individual_completed`; `ExperimentManagerPanel`; `frozen_individual_config_payload`; `result_contracts.py`; `ExperimentWorker`; Individual v2 and Workspace v3 schemas; focused unit/GUI/integration/restore contracts | Experiment > Individual experiment owns its setup ledger, repetitions, exact verified reuse, direct audit/result contract, and immutable full-stage plan without consuming Portfolio or Workspace Study records. Workspace alone consumes the current Portfolio goal, Study recommendation/selection, outputs, case matrix, and automated cells through the same ExperimentManager. Controller exclusivity, Audit -> Stage -> Run, scientific semantics, and policy gates remain intact. Schema-v36 complete engineering PASS; schema-v37 binds ledger closure. |
| One-way Portfolio goal to Workspace Study | `PortfolioGoalPlanner`; `WorkspaceStudyPlanner`; `AppliedPortfolioGoal`; `AppliedStudySetup`; Workspace plan v3; schema-v3 `portfolio_goals` and `applied_study_setups`; Portfolio and Experiment Manager panels | Portfolio Apply persists only broad immutable intent and never changes exact runs or creates a plan. Workspace Study requires the exact current goal, exposes deterministic hard minima/recommendations, records scientist overrides, and creates/replaces only an unstarted draft on explicit Apply Study. New goals or stages invalidate unstarted downstream state; retained lifecycle plans remain immutable. Legacy combined rows are not relabeled. | Schema-v36 complete engineering PASS; schema-v37 binds ledger closure; no scientific, usability, publication, or release claim |
| Individual navigation deduplication and action guidance | `StudySetupWorkflow.step_tabs`; `ExperimentManagerPanel._apply_mode_presentation`; `refresh_execution_state`; `test_workspace_execution_ui.py` | Individual hides the redundant inline step strip because the Experiment ribbon owns the six routes; Workspace Study retains it. Disabled Stage and Run actions explain the exact missing persisted transition. The numerical-agreement action is explicitly optional, parity-only success stores no fairness receipt, and the visible execution message requires fairness -> Stage -> Run. Setup completion and parity never substitute for receipt or controller ownership. | Schema-v38 complete engineering PASS; schema-v39 binds ledger closure; no live audit, experiment, scientific, policy, usability, or release claim |
