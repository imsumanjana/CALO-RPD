...
| Versioned TSH-CALO candidate lifecycle | Locally verified through unqualified evaluation and immutable qualification receipt | `tsh_calo_policy_artifact.py`, `tsh_calo_qualification.py`, `tsh_calo_qualification_campaign.py`, algorithm-aware registry; exact ABI/provenance; protected-holdout rejection; candidate-only registration; non-serializable qualification-only evaluation capability; OS-released single-writer campaign lease; failed-integrity resume rejection; screening cannot issue receipts; formal receipt requires direct A–E evidence; activation/binding revalidate the receipt | Execute a new valid real screening identity and A–E ablations, then formal qualification if every frozen criterion passes; the first raced screening attempt is barred from scientific use |
