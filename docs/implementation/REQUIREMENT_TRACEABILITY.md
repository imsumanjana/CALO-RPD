...
| Independent TSH-CALO policy training | PPO, canonical-reward collector, counted development environment and Safe-80 trainer admission locally verified; physical execution pending | `tsh_calo_training.py`, `tsh_calo_training_environment.py`, `tsh_calo_training_resources.py`; v2 counted/Safe-80 ABI; independent training/environment design hashes; loaded-case checksum/formulation binding; protected identity exclusion; counted full batches and topology state; canonical transition rewards; exact FE/scenario-call/resume; solver-failure poisoning; hash-bound resource envelope; deterministic working-set estimate; CUDA-first current-free-VRAM and governed current-available-RAM admission; device leases/allocator ceiling; exact-device resume; immutable candidate device provenance; E mask; pre-solve F rejection; unqualified-only export; no experiment/registry/activation/inference authority | Execute fresh independent ensemble-member training and retain physical CPU/CUDA provenance; no trained candidate exists yet |
...
- Active development suite after the Safe-80 independent trainer boundary, excluding the
  deliberately stale v6.9 release-integrity file: **563 passed, 63 skipped**. The preceding complete-tree checkpoint
...
off. The next legal step is fresh development-only ensemble-member training orchestration followed
by formal qualification against preregistered evidence. No protected case may enter training or
tuning. Paired-seed component ablations must retain the disabled baseline and may not weaken
