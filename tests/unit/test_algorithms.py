...
class SphereProblem:
    ...
    def evaluate(self, x):
        ...
        return Evaluation(
            value, True, 0, {"sphere": value}, {f"x{i}": float(v) for i, v in enumerate(x)}
        )
...
def test_all_primary_algorithms_smoke(name):
    ...
    params = dict(SPECS[name].default_parameters)
    params["use_ai"] = False if name == "CALO" else params.get("use_ai", False)
