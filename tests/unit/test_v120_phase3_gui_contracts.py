...
def test_active_status_records_phase6_development_complete_after_combined_pass():
    ...
    assert payload["phase_6_goal"] == (
        "completed_training_resource_readiness_parity_correction_validation_pending"
    )
    assert payload["phase_6_coding"] == (
        "implemented_shared_safe80_preflight_and_transition_bound_rollout_envelope_awaiting_validation"
    )
    ...
    assert payload["phase_6_validation"] == (
        "passed_phase6-20260813-215626_superseded_by_training_readiness_resource_parity_followup_validation_pending"
    )
