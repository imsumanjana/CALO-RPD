...
pytestmark = pytest.mark.skipif(VERSION != "5.7.0", reason="historical v5.7.0 release gate")


def test_release_identity_matches_pyproject_and_current_freeze_name():
    root = Path(__file__).resolve().parents[2]
    payload = tomllib.loads((root / "pyproject.toml").read_text(encoding="utf-8"))
    assert payload["project"]["version"] == VERSION == "5.7.0"
    assert FREEZE_MANIFEST == "calo_v570_freeze.json"
