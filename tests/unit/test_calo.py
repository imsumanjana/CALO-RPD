...
def test_cognitive_state_and_policy():
    ...
    ev=[Evaluation(float(i+1),True,0,metadata={"constraint_components":{}}) for i in range(8)]
    state=build_cognitive_state(
        pop,ev,epsilon=0.0,previous_best_violation=1.0,previous_best_objective=10.0,
        constraint_stagnation=.2,objective_stagnation=.1,remaining_budget=.8,
        operator_credit=np.full(6,1/6),
    )
    assert state.vector().shape==(STATE_DIM,)
    net=CALOPolicyNetwork(STATE_DIM)
    regime_logits,operator_logits,alpha,beta,value=net(torch.tensor(state.vector(),dtype=torch.float32))
