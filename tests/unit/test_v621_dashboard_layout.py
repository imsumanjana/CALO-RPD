...
def test_dashboard_has_page_scroll_and_four_scrollable_tabs():
    ...
    assert 'self.dashboard_tabs.addTab(_scrollable_tab(study_tab), "Study Setup")' in source
    assert (
        'self.dashboard_tabs.addTab(_scrollable_tab(readiness_tab), "System Readiness")' in source
    )
    assert 'self.dashboard_tabs.addTab(_scrollable_tab(training_tab), "Training Queue")' in source
    assert (
        'self.dashboard_tabs.addTab(_scrollable_tab(context_tab), "Scientific Context")' in source
    )
    assert source.count("self.dashboard_tabs.addTab(") == 4
